...
namespace Nexus.ViewModels.Pages
{
    public class MeetingPageViewModel : ViewModelBase, IDisposable
    {
        ...
        public event Action<MeetingRoomViewModel>? RequestShowMeetingRoom;
        ...
        private async Task StartReceivingVideoAsync(JoinMeetingResponse meetingInfo)
        {
            try
            {
                System.Diagnostics.Debug.WriteLine($"[MeetingPageViewModel] 启动视频接收: 端口={meetingInfo.BroadcastPort}, 密钥前10字符={(meetingInfo.MeetingKey?.Length > 10 ? meetingInfo.MeetingKey.Substring(0, 10) + "..." : meetingInfo.MeetingKey ?? "null")}");
                
                _meetingRoomViewModel = new MeetingRoomViewModel();
                _meetingRoomViewModel.MeetingTitle = meetingInfo.Title;
                _meetingRoomViewModel.LeaveRequested += OnLeaveRequested;
                ...
                RequestShowMeetingRoom?.Invoke(_meetingRoomViewModel);
            }
            catch (Exception ex)
            {
        ...
        {
            _meetingRoomViewModel?.UpdateFrame(frame);
        }
        ...
        private async Task LeaveMeetingAsync()
        {
            ...
            try
            {
                ...
                _meetingRoomViewModel?.Dispose();
                _meetingRoomViewModel = null;
