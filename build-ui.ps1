...
if (Test-Path $ISCC_PATH) {
    ...
    Write-Host "[WARN] 未找到 Inno Setup 6 (ISCC.exe)，跳过安装程序创建" -ForegroundColor Yellow
    Write-Host "[INFO] 请安装 Inno Setup 6 以生成 .exe 安装包，当前仅输出便携版 ZIP" -ForegroundColor Yellow
}
...
if ($UPLOAD_GITHUB) {
    ...
    if ([string]::IsNullOrEmpty($RELEASE_NOTES)) {
        if ($installerCreated) {
            ...
            $RELEASE_NOTES = "## Nexus v$VERSION 发布`n`n### 下载`n- Nexus-$VERSION-win-x64.zip - 便携版`n`n### 系统要求`n- Windows x64`n- .NET 8.0 运行时 (已包含)"
        }
    }
    ...
    if ($installerCreated) {
        ...
        gh release create "v$VERSION" "$OUTPUT_DIR\Nexus-$VERSION-win-x64.zip" --title "Nexus v$VERSION" --notes $RELEASE_NOTES
    }
    ...
    if ($installerCreated) { Write-Host "安装程序: $OUTPUT_DIR\Nexus-$VERSION-win-x64.exe" -ForegroundColor Cyan }
